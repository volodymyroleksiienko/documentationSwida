...
public interface TreeStorageJPA extends JpaRepository<TreeStorage,Integer> {
    ...
    @Query("select t from  TreeStorage  t " +
            "left join QualityStatisticInfo q on q.treeStorage.id=t.id " +
            "where t.breedOfTree.id=?1 and  t.userCompany.id=?2 and t.statusOfTreeStorage=?3 and t.isMainStorage=?4 ")
