...
public interface DryStorageJPA extends JpaRepository<DryStorage,Integer> {
    ...
    @Query("select r from  DryStorage  r where r.breedOfTree.id=?1 and  r.userCompany.id=?2 and  r.extent<>'0.000' and  r.extent not like '-%'")
    ...
    @Query("select obj.extent from DryStorage obj where obj.breedOfTree.id=?1 and obj.breedDescription in ?2 and obj.sizeOfHeight in ?3 and obj.userCompany.contrAgent.id in ?4 and obj.statusOfEntity='ACTIVE' and obj.extent<>'0.000' and  obj.extent not like '-%'")
