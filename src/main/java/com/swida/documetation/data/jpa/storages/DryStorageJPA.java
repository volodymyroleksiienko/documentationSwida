...
public interface DryStorageJPA extends JpaRepository<DryStorage,Integer> {
    ...
    List<DryStorage> findByIdIn(List<Integer> idList);
    ...
    //select of extent
    @Query("select obj.extent from DryStorage obj where obj.breedOfTree.id=?1 and trim(obj.breedDescription) in ?2 and obj.sizeOfHeight in ?3 and obj.sizeOfWidth in ?4 and obj.sizeOfLong in ?5 and obj.userCompany.contrAgent.id in ?6 and obj.statusOfEntity='ACTIVE' and obj.extent<>'0.000' and  obj.extent not like '-%'")
    List<String> getExtent(int breedId,String[] breedDesc,String[] sizeHeight,String[] sizeWidth,String[] sizeLong,int[] agentId);

    @Query("select obj.extent from DryStorage obj where obj.breedOfTree.id=?1 and trim(obj.breedDescription) in ?2 and obj.sizeOfHeight in ?3 and obj.sizeOfLong in ?4 and obj.userCompany.contrAgent.id in ?5 and obj.statusOfEntity='ACTIVE' and obj.extent<>'0.000' and  obj.extent not like '-%'")
